...
function App() {
  ...
  const [view, setView] = useState('home'); // 'home', 'about', 'policies', 'admin'
  ...
  const handleNavigation = (destination: string) => {
    setIsMenuOpen(false);
    if (['home', 'services', 'training', 'corporate', 'internships', 'staffing', 'reviews', 'contact'].includes(destination)) {
      if (view !== 'home') {
        setView('home');
        setTimeout(() => scrollToSection(destination === 'home' ? 'home' : destination), 100);
      } else {
        scrollToSection(destination === 'home' ? 'home' : destination);
      }
    } else if (destination === 'career-upskilling') {
      if (view !== 'home') {
        setView('home');
        setTimeout(() => scrollToSection('internships'), 100);
      } else {
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl animate-in fade-in duration-300">
           <div className="w-full max-w-5xl bg-[#0d1117] rounded-xl overflow-hidden shadow-2xl border border-slate-700 flex flex-col h-[80vh]">
              <div className="bg-slate-800 p-2 flex items-center justify-between border-b border-black">
                 ...
                 <button onClick={() => setActiveLab(null)} className="text-slate-400 hover:text-white transition-colors mr-2">
                    <X size={18} />
                 </button>
              </div>
              <div className="flex-1 bg-black p-6 font-mono text-sm overflow-y-auto font-light leading-6">
                 {labBootLines.map((line, idx) => (
                    <div key={idx} className="text-green-500/90 mb-1">
                       <span className="text-blue-500 mr-2">root@techskyline:~#</span>{line}
                    </div>
                 ))}
                 {isLabReady && (
                    <div className="mt-4 p-4 border border-green-500/30 bg-green-900/10 rounded-lg animate-pulse">
                       ...
                       <p className="text-slate-300">Welcome to the <strong>{activeLab.name}</strong> interactive lab environment.<br/>All tools and dependencies have been pre-loaded.</p>
        ...
        {isMenuOpen && (
          <div className="lg:hidden bg-slate-900/95 backdrop-blur-xl border-b border-white/10 absolute w-full shadow-2xl h-[calc(100vh-6rem)] overflow-y-auto">
            <div className="flex flex-col p-4 space-y-4">
              {['Home', 'Services', 'Training', 'Corporate', 'Internships', 'Staffing', 'Reviews', 'Contact', 'About', 'Policies'].map((item) => (
                <button 
                  key={item}
                  onClick={() => handleNavigation(item.toLowerCase())}
      ...
      <div className="flex-grow">
        {view === 'about' ? (
          ...
          <div className="min-h-screen pt-32 pb-20 container mx-auto px-4 text-center">
             <div className="bg-slate-800/50 p-12 rounded-3xl border border-white/10 inline-block">
                <Lock size={64} className="mx-auto text-slate-500 mb-6" />
                <h1 className="text-4xl font-bold text-white mb-4">Admin Dashboard</h1>
                <p className="text-slate-400 text-lg">Access to this area requires authentication.</p>
                <button onClick={() => handleNavigation('home')} className="mt-8 px-6 py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg font-bold transition-colors">Return Home</button>
             </div>
          </div>
        ) : (
          /* HOME VIEW CONTENT */
          <>
                ...
                <div className="max-w-5xl mx-auto flex flex-col items-center text-center">
                  <div className="mb-10 relative group">
                    ...
                    <div className="relative bg-black/40 backdrop-blur-sm p-6 rounded-3xl border border-white/10 shadow-2xl">
                      <img src={CONTACT_INFO.heroLogo} alt="Tech Skyline IT Solutions" className="h-32 md:h-48 w-auto object-contain drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]"/>
                      <div className="mt-4 text-center">
                          <h1 className="text-4xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 via-yellow-500 to-yellow-200 tracking-tighter drop-shadow-sm uppercase">TECH SKYLINE</h1>
                          <p className="text-xl md:text-2xl text-slate-300 font-bold tracking-[0.4em] mt-2 border-t border-white/20 pt-2 inline-block">IT SOLUTIONS</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-6 justify-center w-full sm:w-auto">
                    <button onClick={() => scrollToSection('contact')} className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white px-8 py-4 rounded-xl font-bold transition-all shadow-[0_0_20px_rgba(6,182,212,0.5)] flex items-center justify-center gap-3 transform hover:-translate-y-1 hover:scale-105">Start Your Journey <ArrowRight size={20} /></button>
                    <button onClick={() => scrollToSection('services')} className="bg-white/5 backdrop-blur-md border border-white/20 text-white hover:bg-white/10 px-8 py-4 rounded-xl font-bold transition-all transform hover:-translate-y-1 flex items-center justify-center">Explore Services</button>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="py-20 relative overflow-hidden">
              <div className="container mx-auto px-4 md:px-6 relative z-10">
                ...
                <div className="relative max-w-6xl mx-auto">
                  <button onClick={prevSlide} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 md:-translate-x-12 z-20 bg-slate-800/80 backdrop-blur-md p-3 rounded-full shadow-lg text-white hover:bg-cyan-600 transition-all focus:outline-none border border-white/10"><ChevronLeft size={24} /></button>
                  <button onClick={nextSlide} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 md:translate-x-12 z-20 bg-slate-800/80 backdrop-blur-md p-3 rounded-full shadow-lg text-white hover:bg-cyan-600 transition-all focus:outline-none border border-white/10"><ChevronRight size={24} /></button>
                  <div className="overflow-hidden px-2 py-4">
                    <div className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentSlide * (100 / itemsPerSlide)}%)` }}>
                      {FEATURED_TECHNOLOGIES.map((tech, index) => (
                        <div key={index} className="flex-shrink-0 px-4" style={{ width: `${100 / itemsPerSlide}%` }}>
                          <div className="group relative overflow-hidden rounded-2xl bg-slate-800 shadow-xl hover:shadow-cyan-500/20 transition-all duration-300 h-full border border-white/10">
                            <div className="aspect-[4/3] overflow-hidden relative">
                              <img src={tech.image} alt={tech.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"/>
            ...
            <section id="services" className="py-20 bg-black/20 backdrop-blur-sm">
              <div className="container mx-auto px-4 md:px-6">
                <div className="text-center mb-16">
                  ...
                  <p className="text-slate-400 mt-4 max-w-2xl mx-auto">Expertly guiding your IT and digital transformation initiatives from strategy to implementation.</p>
                </div>
                  ...
                  {CONSULTING_SERVICES.map((service, index) => (
                    <div key={index} className="bg-white/5 backdrop-blur-md rounded-2xl overflow-hidden shadow-lg hover:shadow-cyan-500/20 transition-all duration-300 border border-white/10 group">
                      ...
                      <div className="p-8">
                        ...
                        <button onClick={() => scrollToSection('contact')} className="inline-flex items-center text-cyan-400 font-bold text-sm hover:text-cyan-300 transition-colors">Learn more <ChevronRight size={16} /></button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="container mx-auto px-4 md:px-6 relative z-10">
                <div className="flex flex-col lg:flex-row gap-16">
                  <div className="lg:w-1/3">
                    ...
                    <p className="text-slate-300 mb-8 leading-relaxed font-light">We provide 100+ courses online with interactive training. Our courses are designed by industry experts to equip students with skills they can immediately apply in the workplace.</p>
                    <div className="bg-gradient-to-br from-purple-900/50 to-indigo-900/50 backdrop-blur-md p-6 rounded-2xl border border-white/10 mb-8 hover:bg-white/5 transition-colors shadow-lg">
                      ...
                      <p className="text-sm text-slate-300 mb-4 leading-relaxed">{CERTIFICATION_TEXT.substring(0, 150)}...</p>
                    ...
                    <div className="mb-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {TESTING_SERVICES.map((test, idx) => (
                          <span key={idx} className="bg-white/5 text-slate-300 px-3 py-1.5 rounded-lg text-xs border border-white/10 hover:border-cyan-500/50 hover:bg-cyan-500/10 hover:text-cyan-300 transition-all cursor-default font-medium">{test}</span>
                        ))}
                      </div>
                    </div>
                  </div>
                  <div className="lg:w-2/3">
                    <div className="flex flex-wrap gap-2 mb-6 items-center">
                        ...
                        <button onClick={() => setActiveCourseFilter("All")} className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all border ${activeCourseFilter === "All" ? "bg-purple-600 text-white border-purple-500 shadow-lg shadow-purple-500/25" : "bg-slate-800/50 text-slate-400 border-white/5 hover:bg-slate-800 hover:text-white"}`}>All</button>
                        {COURSE_CATEGORIES.map((cat) => (
                          <button key={cat.name} onClick={() => setActiveCourseFilter(cat.name)} className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all border ${activeCourseFilter === cat.name ? "bg-purple-600 text-white border-purple-500 shadow-lg shadow-purple-500/25" : "bg-slate-800/50 text-slate-400 border-white/5 hover:bg-slate-800 hover:text-white"}`}>{cat.name.includes("Cloud") ? "Cloud & DevOps" : cat.name.includes("AI") ? "AI & Data Science" : cat.name.includes("Enterprise") ? "Enterprise" : "Development"}</button>
                        ))}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {COURSE_CATEGORIES.filter(category => activeCourseFilter === "All" || category.name === activeCourseFilter).map((category, index) => (
                        <div key={index} className="bg-slate-800/40 backdrop-blur-sm p-6 rounded-2xl border border-white/5 hover:border-purple-500/50 transition-all group hover:-translate-y-1 shadow-lg animate-in fade-in zoom-in duration-300">
                          ...
                          <ul className="space-y-3">
                            {category.courses.map((course, cIdx) => (
                              <li key={cIdx} className="flex items-center text-slate-300 text-sm group-hover:text-white transition-colors"><span className="w-1.5 h-1.5 bg-purple-500 rounded-full mr-3 shadow-[0_0_8px_rgba(168,85,247,0.6)]"></span>{course}</li>
                            ))}
                          </ul>
                        </div>
                      ))}
            ...
            <section id="corporate" className="py-20 relative bg-slate-900 border-t border-white/5 overflow-hidden">
              ...
              <div className="container mx-auto px-4 md:px-6 relative z-10">
                  <div className="text-center mb-16 max-w-4xl mx-auto">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-blue-900/50 border border-blue-500/30 text-blue-300 rounded-full text-xs font-bold uppercase tracking-wider mb-6"><BriefcaseIcon size={14} /> For Organizations</div>
                    ...
                    <p className="text-slate-400">{CORPORATE_TRAINING.description}</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
                    {CORPORATE_TRAINING.benefits.map((benefit, idx) => (
                        <div key={idx} className="bg-white/5 p-6 rounded-2xl border border-white/5 hover:bg-white/10 hover:border-blue-500/30 transition-all group">
                          <div className="w-12 h-12 bg-slate-800 rounded-xl flex items-center justify-center text-blue-400 group-hover:text-white group-hover:bg-blue-600 transition-colors mb-4 shadow-lg"><benefit.icon size={24} /></div>
                  ...
                  <div className="flex flex-col lg:flex-row gap-8">
                    <div className="lg:w-5/12 bg-slate-800/50 p-8 rounded-3xl border border-white/5">
                        <h3 className="text-2xl font-bold text-white mb-8 flex items-center gap-3"><LayoutList className="text-cyan-400" /> Implementation Model</h3>
                        <div className="space-y-6">
                          {CORPORATE_TRAINING.process.map((step, idx) => (
                              <div key={idx} className="flex gap-4 relative group">
                                {idx < CORPORATE_TRAINING.process.length - 1 && <div className="absolute left-6 top-10 bottom-[-24px] w-0.5 bg-white/10 group-hover:bg-cyan-500/30 transition-colors"></div>}
                                <div className="relative z-10 w-12 h-12 rounded-full bg-slate-900 border border-white/10 flex items-center justify-center shrink-0 group-hover:border-cyan-500 group-hover:shadow-[0_0_10px_rgba(6,182,212,0.3)] transition-all"><span className="text-xs font-bold text-slate-500 group-hover:text-cyan-400">{step.step}</span></div>
                                <div className="pt-2"><h4 className="text-lg font-bold text-white mb-1 group-hover:text-cyan-400 transition-colors">{step.title}</h4><p className="text-sm text-slate-400">{step.desc}</p></div>
                              </div>
                          ))}
                        </div>
                    </div>
                    <div className="lg:w-7/12 flex flex-col gap-6">
                        <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-8 rounded-3xl border border-white/5 flex-1">
                          <h3 className="text-2xl font-bold text-white mb-6 flex items-center gap-3"><Cpu className="text-purple-400" /> Technology Capabilities</h3>
                          <div className="space-y-6">
                              {CORPORATE_TRAINING.technologies.map((tech, idx) => (
                                <div key={idx} className="border-b border-white/5 pb-4 last:border-0 last:pb-0"><h4 className="text-sm font-bold text-slate-400 uppercase tracking-wide mb-3">{tech.category}</h4><div className="flex flex-wrap gap-2">{tech.items.map((item, i) => (<span key={i} className="px-3 py-1.5 bg-slate-700/50 hover:bg-purple-600 hover:text-white rounded-lg text-xs font-medium text-slate-300 transition-all cursor-default border border-white/5">{item}</span>))}</div></div>
                              ))}
                          </div>
                        </div>
                        <div className="bg-gradient-to-r from-blue-600 to-cyan-600 p-8 rounded-3xl shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6">
                          <div className="text-white"><h4 className="text-xl font-bold mb-2">Ready to upskill your team?</h4><p className="text-blue-100 text-sm">Get a customized training proposal for your organization.</p></div>
                          <button onClick={() => scrollToSection('contact')} className="px-6 py-3 bg-white text-blue-700 hover:bg-slate-100 font-bold rounded-xl transition-colors shadow-lg whitespace-nowrap">Request Corporate Brochure</button>
                        </div>
                    </div>
                  </div>
              </div>
            ...
            <section id="internships" className="py-20 bg-gradient-to-b from-white/5 to-transparent relative overflow-hidden backdrop-blur-sm">
              <div className="container mx-auto px-4 md:px-6 relative z-10">
                <div className="text-center mb-12">
                  <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-gradient-to-r from-cyan-500 to-blue-600 shadow-lg text-white rounded-full text-xs font-bold uppercase tracking-wider mb-6"><Laptop size={14} /> For Students & Self-Learners</div>
                ...
                <div className="flex flex-col gap-12 items-start">
                  <div className="w-full">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
                      {INTERNSHIP_PROGRAM.features.map((feat, idx) => (
                        <div key={idx} className="bg-slate-900/60 backdrop-blur-md p-6 rounded-2xl shadow-lg border border-white/5 hover:border-cyan-500/30 transition-all flex gap-4 items-start group">
                          <div className="bg-slate-800 p-3 rounded-xl shrink-0 shadow-lg text-cyan-400 group-hover:text-white group-hover:bg-cyan-600 transition-all"><feat.icon size={24} /></div>
                          <div><h4 className="font-bold text-white mb-2">{feat.title}</h4><p className="text-sm text-slate-400 font-light">{feat.desc}</p></div>
                        </div>
                      ))}
                    </div>
                    <div className="mb-10 bg-white/5 backdrop-blur-md p-8 rounded-3xl border border-white/10">
                        <h4 className="font-bold text-white mb-8 flex items-center gap-3 text-xl"><LayoutList className="text-purple-400" size={24} /> Program Structure</h4>
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                          {INTERNSHIP_PROGRAM.programSteps.map((step, idx) => (
                            <div key={idx} className="bg-slate-800/50 p-5 rounded-2xl border border-white/5 text-center relative group hover:bg-slate-800 hover:border-purple-500/50 hover:shadow-xl transition-all hover:-translate-y-1">
                                ...
                                {idx < INTERNSHIP_PROGRAM.programSteps.length - 1 && (<div className="hidden md:block absolute top-1/2 -right-3 text-slate-600 transform -translate-y-1/2 z-10"><ChevronRight size={20} /></div>)}
                            </div>
                          ))}
                        </div>
                    </div>
                    <div className="bg-slate-900/40 backdrop-blur-md p-8 rounded-3xl border border-white/10 mb-10">
                      <h4 className="font-bold text-xl text-white mb-6 flex items-center gap-3"><Zap className="text-yellow-400 fill-yellow-400" /> Technologies & Live Labs</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {INTERNSHIP_PROGRAM.technologies.map((tech, idx) => (
                          <div key={idx} className="bg-slate-800/50 p-4 rounded-xl border border-white/5 flex flex-col justify-between hover:bg-slate-800 hover:border-cyan-500/30 transition-all group">
                            <div className="mb-4">
                                <div className="flex justify-between items-start mb-2"><h5 className="font-bold text-slate-200 group-hover:text-white transition-colors">{tech.name}</h5><span className="px-2 py-0.5 bg-white/5 text-[10px] rounded text-slate-400 border border-white/5">{tech.type}</span></div>
                                <div className="flex items-center gap-2 text-xs text-green-400"><span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>Lab Environment Active</div>
                            </div>
                            <button onClick={() => handleLabLaunch(tech)} className="w-full bg-cyan-600/20 hover:bg-cyan-500 hover:text-white text-cyan-400 border border-cyan-500/30 rounded-lg py-2 px-3 text-xs font-bold uppercase tracking-wide transition-all flex items-center justify-center gap-2 group-hover:shadow-lg group-hover:shadow-cyan-500/20"><Terminal size={14} /> Launch Live Lab</button>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div className="bg-gradient-to-r from-slate-900/90 to-indigo-900/90 backdrop-blur-xl p-8 md:p-10 rounded-3xl border border-white/10 shadow-2xl overflow-hidden relative mb-10">
                      <div className="absolute top-0 right-0 p-4 opacity-5"><Laptop size={200} className="text-white" /></div>
                      ...
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 relative z-10">
                          <div>
                              <h4 className="font-bold text-2xl text-white mb-2 flex items-center gap-3"><Lock size={24} className="text-cyan-400" /> Student Portal & Resource Access</h4>
                              <p className="text-slate-400 text-sm max-w-xl font-light">Unlock instant access to our comprehensive digital learning ecosystem including labs, projects, and portfolio tools.</p>
                          </div>
                          <button onClick={openGoogleForm} className="mt-6 md:mt-0 bg-white text-slate-900 hover:bg-cyan-400 hover:text-slate-900 px-8 py-3 rounded-xl font-bold text-sm shadow-xl flex items-center gap-2 transition-all transform hover:-translate-y-1">Register for Access <ExternalLink size={16} /></button>
                      </div>
                        ...
                        {INTERNSHIP_PROGRAM.studentResources?.map((res, idx) => (
                          <div key={idx} className="bg-white/5 backdrop-blur-md border border-white/10 p-5 rounded-2xl hover:bg-white/10 hover:border-cyan-400/30 transition-all group cursor-pointer h-full hover:-translate-y-1 shadow-lg">
                              <div className="flex flex-col gap-3 h-full justify-between">
                                <div>
                                      <div className="bg-slate-800 p-2.5 rounded-xl group-hover:bg-cyan-500 group-hover:text-white transition-colors text-cyan-400 w-fit mb-4 shadow-inner"><res.icon size={22} /></div>
                                ...
                                <span className="text-xs font-bold text-cyan-400 uppercase tracking-wide flex items-center gap-1 mt-auto group-hover:text-white transition-colors">{res.linkText} <ArrowRight size={12} className="group-hover:translate-x-1 transition-transform" /></span>
                              </div>
                          </div>
                        ))}
                      </div>
              ...
              <div className="container mx-auto px-4 md:px-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {SAMPLE_CLASSES.map((cls) => (
                      <div key={cls.id} onClick={() => handleClassSelect(cls)} className="bg-slate-800 rounded-xl overflow-hidden shadow-lg border border-white/5 hover:border-cyan-500/50 hover:shadow-cyan-500/20 transition-all cursor-pointer group hover:-translate-y-1">
                        <div className="h-40 relative overflow-hidden bg-black">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                              <div className="bg-white/10 backdrop-blur-sm p-3 rounded-full border border-white/20 group-hover:bg-cyan-500 group-hover:border-cyan-500 transition-colors shadow-lg">{cls.type === 'Video' ? <Play size={24} className="fill-white text-white pl-1"/> : <Headphones size={24} className="text-white"/>}</div>
                            </div>
                            <span className="absolute bottom-2 right-2 bg-black/70 text-white text-[10px] px-2 py-0.5 rounded font-mono border border-white/10">{cls.duration}</span>
                            <span className={`absolute top-2 left-2 text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ${cls.type === 'Video' ? 'bg-purple-600' : 'bg-emerald-600'}`}>{cls.type}</span>
                        </div>
                        <div className="p-5">
                            ...
                            <div className="flex justify-between items-center border-t border-white/5 pt-3"><p className="text-xs text-slate-400">{cls.author}</p><span className="text-[10px] text-cyan-400 uppercase font-bold tracking-wider">Play Now</span></div>
                        </div>
                      </div>
                  ))}
                </div>
                ...
                <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-white/10">
                  <div className="flex flex-col md:flex-row gap-16 items-center">
                    <div className="md:w-1/2 order-2 md:order-1">
                      ...
                      <p className="text-slate-300 mb-8 leading-relaxed font-light">We enable you to achieve and optimize the most strategic and variable component to business success—right people, with right skills, competencies, and attitudes.</p>
                      <div className="space-y-6">
                        {STAFFING_FEATURES.map((feature, index) => (
                          <div key={index} className="flex gap-4 group">
                            <div className="mt-1 bg-slate-800 p-3 rounded-xl text-cyan-400 h-fit group-hover:bg-cyan-600 group-hover:text-white transition-all shadow-lg"><feature.icon size={20} /></div>
                            <div><h4 className="font-bold text-white text-lg group-hover:text-cyan-400 transition-colors">{feature.title}</h4><p className="text-slate-400 text-sm mt-1 leading-relaxed">{feature.description}</p></div>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div className="md:w-1/2 order-1 md:order-2">
                      <div className="relative">
                        ...
                        <img src="https://images.unsplash.com/photo-1560250097-0b93528c311a?auto=format&fit=crop&w=800&q=80" alt="Professional Staffing" className="rounded-2xl shadow-2xl relative z-10 border border-white/10"/>
                        <div className="absolute bottom-8 left-8 bg-slate-900/90 backdrop-blur-md p-6 rounded-xl shadow-2xl max-w-xs hidden sm:block border border-white/20 z-20">
                          <div className="flex items-center gap-3 mb-2"><Users className="text-cyan-400" /><span className="font-bold text-white text-2xl">98.8%</span></div>
            ...
            <section id="reviews" className="py-20 relative bg-black/20 backdrop-blur-sm">
              <div className="container mx-auto px-4 md:px-6">
                <div className="text-center mb-16"><h2 className="text-sm font-bold text-cyan-400 uppercase tracking-widest mb-2">Testimonials</h2><h3 className="text-3xl font-bold text-white">What People Say</h3></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {TESTIMONIALS.map((testimonial) => (
                    <div key={testimonial.id} className="bg-slate-800/50 p-8 rounded-2xl border border-white/5 relative hover:bg-slate-800 transition-colors">
                      ...
                      <div className="flex gap-1 mb-4">{[...Array(testimonial.rating)].map((_, i) => (<Star key={i} size={16} className="fill-yellow-400 text-yellow-400" />))}</div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div><h4 className="font-bold text-white text-sm">{testimonial.author}</h4><p className="text-xs text-slate-400">{testimonial.role}</p></div>
                      </div>
                      <div className="mt-4 pt-4 border-t border-white/5"><span className="text-[10px] font-bold px-2 py-1 rounded bg-white/5 text-cyan-400 border border-white/5">{testimonial.category}</span></div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="bg-slate-900/80 backdrop-blur-xl rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
                  <div className="flex flex-col lg:flex-row">
                    <div className="lg:w-1/3 bg-gradient-to-br from-cyan-600 to-blue-700 p-10 text-white relative overflow-hidden">
                      ...
                      <p className="text-blue-100 mb-8 relative z-10 leading-relaxed">Ready to transform your business or career? Contact us today for a free consultation.</p>
                      <div className="space-y-6 relative z-10">
                        <div className="flex items-start gap-4"><Phone className="mt-1 shrink-0 text-blue-200" size={20} /><div><h4 className="font-bold text-sm">Phone</h4><p className="text-blue-100 text-sm">{CONTACT_INFO.phone}</p><p className="text-blue-200 text-xs mt-1">Mon-Fri 9am-6pm EST</p></div></div>
                        <div className="flex items-start gap-4"><Mail className="mt-1 shrink-0 text-blue-200" size={20} /><div><h4 className="font-bold text-sm">Email</h4><p className="text-blue-100 text-sm">{CONTACT_INFO.email}</p></div></div>
                        <div className="flex items-start gap-4"><MapPin className="mt-1 shrink-0 text-blue-200" size={20} /><div><h4 className="font-bold text-sm">Office</h4><p className="text-blue-100 text-sm">{CONTACT_INFO.address}</p></div></div>
                      </div>
                      <div className="mt-12 relative z-10">
                        <h4 className="font-bold text-sm mb-4">Connect With Us</h4>
                        <div className="flex gap-3"><button onClick={() => window.open(CONTACT_INFO.googleFormUrl, '_blank')} className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/30 transition-colors"><Globe size={18} /></button></div>
                      </div>
                    </div>
                    <div className="lg:w-2/3 p-10">
                      ...
                      <form className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div><label className="block text-xs font-bold text-slate-400 uppercase mb-1">Name</label><input type="text" name="name" value={formData.name} onChange={handleInputChange} className={`w-full bg-slate-800 border ${errors.name ? 'border-red-500' : 'border-white/10'} rounded-lg px-4 py-3 text-white focus:outline-none focus:border-cyan-500 transition-colors`} placeholder="Your Name" />{errors.name && <p className="text-red-500 text-xs mt-1">{errors.name}</p>}</div>
                          <div><label className="block text-xs font-bold text-slate-400 uppercase mb-1">Phone</label><input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} className={`w-full bg-slate-800 border ${errors.phone ? 'border-red-500' : 'border-white/10'} rounded-lg px-4 py-3 text-white focus:outline-none focus:border-cyan-500 transition-colors`} placeholder="Your Phone Number" />{errors.phone && <p className="text-red-500 text-xs mt-1">{errors.phone}</p>}</div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div><label className="block text-xs font-bold text-slate-400 uppercase mb-1">Email</label><input type="email" name="email" value={formData.email} onChange={handleInputChange} className={`w-full bg-slate-800 border ${errors.email ? 'border-red-500' : 'border-white/10'} rounded-lg px-4 py-3 text-white focus:outline-none focus:border-cyan-500 transition-colors`} placeholder="Your Email Address" />{errors.email && <p className="text-red-500 text-xs mt-1">{errors.email}</p>}</div>
                          <div><label className="block text-xs font-bold text-slate-400 uppercase mb-1">I am interested in</label><select name="interest" value={formData.interest} onChange={handleInputChange} className="w-full bg-slate-800 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-cyan-500 transition-colors"><option>Corporate Training</option><option>IT Staffing</option><option>Consulting Services</option><option>Student Internship</option><option>Course Enquiry</option><option>Other</option></select></div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                           <div><label className="block text-xs font-bold text-slate-400 uppercase mb-1">Location (Optional)</label><input type="text" name="location" value={formData.location} onChange={handleInputChange} className="w-full bg-slate-800 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-cyan-500 transition-colors" placeholder="City, Country" /></div>
                           <div><label className="block text-xs font-bold text-slate-400 uppercase mb-1">Company / College (Optional)</label><input type="text" name="company" value={formData.company} onChange={handleInputChange} className="w-full bg-slate-800 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-cyan-500 transition-colors" placeholder="Organization Name" /></div>
                        </div>
                        <div><label className="block text-xs font-bold text-slate-400 uppercase mb-1">Subject</label><input type="text" name="subject" value={formData.subject} onChange={handleInputChange} className={`w-full bg-slate-800 border ${errors.subject ? 'border-red-500' : 'border-white/10'} rounded-lg px-4 py-3 text-white focus:outline-none focus:border-cyan-500 transition-colors`} placeholder="Subject" />{errors.subject && <p className="text-red-500 text-xs mt-1">{errors.subject}</p>}</div>
                        <div><label className="block text-xs font-bold text-slate-400 uppercase mb-1">Message</label><textarea name="message" value={formData.message} onChange={handleInputChange} rows={4} className={`w-full bg-slate-800 border ${errors.message ? 'border-red-500' : 'border-white/10'} rounded-lg px-4 py-3 text-white focus:outline-none focus:border-cyan-500 transition-colors resize-none`} placeholder="How can we help you?"></textarea>{errors.message && <p className="text-red-500 text-xs mt-1">{errors.message}</p>}</div>
                        <div className="flex flex-col sm:flex-row gap-4 pt-4">
                          <button onClick={handleWhatsApp} className="flex-1 bg-green-600 hover:bg-green-500 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg hover:shadow-green-500/30 transform hover:-translate-y-1"><MessageCircle size={20} /> Chat on WhatsApp</button>
                          <button onClick={handleEmail} className="flex-1 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg hover:shadow-cyan-500/30 transform hover:-translate-y-1"><Mail size={20} /> Send Inquiry via Email</button>
                        </div>
                        <p className="text-center text-xs text-slate-500 mt-4">By submitting this form, you agree to our privacy policy. We typically respond within 24 hours.</p>
                      </form>
                    </div>
                  </div>
                </div>
         ...
         <div className="container mx-auto px-4">
            <div className="flex flex-col md:flex-row justify-between items-center mb-12">
               ...
               <div className="flex gap-4 flex-wrap justify-center">
                  ...
                  {TECH_STACK.map((tech: any, i: number) => (
            ...
            <div className="flex justify-center gap-4 mb-6">
               {SOCIAL_MEDIA.map((social: any, idx: number) => (
