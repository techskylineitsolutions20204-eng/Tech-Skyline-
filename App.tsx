...
function App() {
  ...
  const handlePortalClick = (resourceTitle: string) => {
    ...
    // Map resource titles to portal views
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl animate-in fade-in duration-300">
           <div className="bg-[#0f172a] w-full max-w-6xl h-[85vh] rounded-2xl overflow-hidden shadow-2xl border border-slate-700 flex flex-col">
              ...
              <div className="bg-slate-800/50 p-4 border-b border-white/10 flex justify-between items-center">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       ...
                       <p className="text-xs text-green-400 flex items-center gap-1"><Lock size={10}/> Authenticated Access</p>
                    </div>
                 </div>
                 <button onClick={() => setActivePortalView(null)} className="p-2 hover:bg-white/10 rounded-full transition-colors text-slate-400 hover:text-white">
                    <X size={24}/>
                 </button>
              </div>
              ...
              <div className="flex flex-1 overflow-hidden">
                 {/* Sidebar */}
                 <div className="w-64 bg-slate-900 border-r border-white/5 p-4 flex-col gap-2 hidden md:flex">
                    {[
                       { id: 'video-archive', label: 'Video Archive', icon: PlayCircle },
                       { id: 'lab-dashboard', label: 'Lab Dashboard', icon: Terminal },
                       { id: 'course-materials', label: 'Course Materials', icon: FileText },
                       { id: 'practice-arena', label: 'Practice Arena', icon: FileCode },
                       { id: 'portfolio-builder', label: 'My Portfolio', icon: UserCheck },
                    ].map((item) => (
                       <button 
                          key={item.id}
                          onClick={() => setActivePortalView(item.id)}
                          className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${activePortalView === item.id ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
                       >
                 ...
                 <div className="flex-1 p-8 overflow-y-auto bg-slate-950/50">
                    ...
                    {/* VIDEO ARCHIVE VIEW */}
                    {activePortalView === 'video-archive' && (
                       <div className="animate-in slide-in-from-right-4 duration-300">
                          <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-2"><VideoIcon className="text-purple-400"/> Recent Session Recordings</h2>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                             {SAMPLE_CLASSES.concat(SAMPLE_CLASSES).map((cls, idx) => (
                                <div key={idx} onClick={() => { setActivePortalView(null); handleClassSelect(cls); }} className="bg-slate-800 rounded-xl overflow-hidden cursor-pointer group hover:ring-2 hover:ring-cyan-500 transition-all border border-white/5">
                                   <div className="h-40 bg-black relative">
                                      <img src={cls.thumbnail} className="w-full h-full object-cover opacity-60 group-hover:opacity-80 transition-opacity" alt="thumbnail"/>
                                      <div className="absolute inset-0 flex items-center justify-center">
                                         <PlayCircle size={40} className="text-white drop-shadow-lg group-hover:scale-110 transition-transform"/>
                                      </div>
                                      <span className="absolute bottom-2 right-2 bg-black/80 text-white text-xs px-2 py-0.5 rounded">{cls.duration}</span>
                                   </div>
                                   <div className="p-4">
                                      <h4 className="font-bold text-slate-200 text-sm mb-1">{cls.title}</h4>
                                      <p className="text-xs text-slate-500">Instructor: {cls.author}</p>
                                   </div>
                                </div>
                             ))}
                          </div>
                    ...
                    {/* LAB DASHBOARD VIEW */}
                    {activePortalView === 'lab-dashboard' && (
                       <div className="animate-in slide-in-from-right-4 duration-300 max-w-4xl mx-auto">
                          <div className="flex justify-between items-end mb-8">
                             <div>
                                <h2 className="text-2xl font-bold text-white mb-1">Cloud Lab Environment</h2>
                                <p className="text-slate-400 text-sm">Instance ID: i-0f9a8b7c6d5e4f3a2 • Region: us-east-1</p>
                             </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                             <div className="bg-slate-800 p-6 rounded-2xl border border-white/5">
                                <div className="text-slate-400 text-xs uppercase font-bold mb-2">CPU Usage</div>
                                <div className="text-3xl font-mono text-cyan-400 mb-2">12%</div>
                             ...
                             <div className="bg-slate-800 p-6 rounded-2xl border border-white/5">
                                ...
                                <div className="text-3xl font-mono text-purple-400 mb-2">4.2 GB</div>
                                <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden">
                                   <div className="h-full bg-purple-500 w-[26%]"></div>
                                </div>
                             </div>
                             <div className="bg-slate-800 p-6 rounded-2xl border border-white/5">
                                <div className="text-slate-400 text-xs uppercase font-bold mb-2">Storage</div>
                                <div className="text-3xl font-mono text-blue-400 mb-2">45%</div>
                                <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden">
                          ...
                          <div className="bg-black rounded-xl p-8 border border-white/10 text-center">
                             <Terminal size={64} className="text-slate-700 mx-auto mb-6"/>
                             <h3 className="text-xl font-bold text-white mb-2">Ready to Code?</h3>
                             <p className="text-slate-400 mb-8 max-w-lg mx-auto">Your development environment is pre-configured with Python, Node.js, Docker, and AWS CLI. Changes are auto-saved.</p>
                             <button 
                                onClick={() => { setActivePortalView(null); handleLabLaunch({ name: 'Student-Dev-Env', id: 'SDE-01' }); }}
                                className="bg-green-600 hover:bg-green-500 text-white font-bold py-3 px-8 rounded-xl shadow-lg shadow-green-900/50 transition-all transform hover:scale-105"
                             >
                    ...
                    {/* MATERIALS VIEW */}
                    {activePortalView === 'course-materials' && (
                       <div className="animate-in slide-in-from-right-4 duration-300">
                           <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-2"><FolderOpen className="text-yellow-400"/> Course Repository</h2>
                           <div className="bg-slate-800 rounded-2xl overflow-hidden border border-white/5">
                              <table className="w-full text-left border-collapse">
                                 ...
                                 <tbody className="divide-y divide-white/5">
                                    {[
                                       { name: "AWS_Solutions_Architect_Study_Guide_v4.pdf", type: "PDF Document", size: "12.4 MB" },
                                       { name: "Python_Advanced_Data_Structures.pdf", type: "PDF Document", size: "4.1 MB" },
                                       { name: "Kubernetes_Cluster_Setup_Scripts.zip", type: "Archive", size: "1.8 MB" },
                                       { name: "React_Hooks_Cheatsheet.png", type: "Image", size: "2.2 MB" },
                                       { name: "Docker_Compose_Templates.yml", type: "Config", size: "14 KB" },
                                       { name: "System_Design_Interview_QnA.docx", type: "Word Doc", size: "850 KB" },
                                    ].map((file, idx) => (
                                       <tr key={idx} className="hover:bg-white/5 transition-colors group">
                                          <td className="p-4 text-slate-300 font-medium flex items-center gap-3">
                                             <FileText size={18} className="text-slate-500 group-hover:text-cyan-400"/> {file.name}
                                          </td>
                                          <td className="p-4 text-slate-400 text-sm">{file.type}</td>
                                          <td className="p-4 text-slate-500 text-sm font-mono">{file.size}</td>
                                          <td className="p-4 text-right">
                                             <button className="text-cyan-400 hover:text-white transition-colors p-2 hover:bg-cyan-500/20 rounded-lg">
                                                <Download size={18}/>
                                             </button>
                                          </td>
                                       </tr>
                    ...
                    {activePortalView === 'practice-arena' && (
                       <div className="animate-in slide-in-from-right-4 duration-300 h-full flex flex-col">
                           <div className="flex justify-between items-center mb-4">
                              <h2 className="text-2xl font-bold text-white flex items-center gap-2"><FileCode className="text-pink-400"/> Coding Playground</h2>
                              <div className="flex gap-2">
                                 <button className="px-3 py-1.5 bg-slate-800 text-slate-300 rounded-lg text-xs font-bold border border-white/10 hover:text-white">Python</button>
                                 <button className="px-3 py-1.5 bg-slate-800 text-slate-300 rounded-lg text-xs font-bold border border-white/10 hover:text-white">JavaScript</button>
                              </div>
                           </div>
                           <div className="flex-1 bg-[#1e1e1e] rounded-xl border border-white/10 overflow-hidden flex flex-col font-mono text-sm shadow-xl">
                              <div className="bg-[#252526] p-2 flex items-center gap-4 border-b border-black text-xs text-slate-400 px-4">
                                 <span>main.py</span>
                                 <span>utils.py</span>
                              </div>
                              <div className="flex-1 p-4 text-slate-300 relative">
                                 <div className="absolute left-0 top-4 bottom-0 w-12 text-right pr-4 text-slate-600 select-none border-r border-slate-700">
                                    1<br/>2<br/>3<br/>4<br/>5<br/>6<br/>7<br/>8
                                 </div>
                                 <div className="pl-8">
                                    <span className="text-pink-400">def</span> <span className="text-yellow-300">binary_search</span>(arr, target):<br/>
                                    &nbsp;&nbsp;left, right = 0, <span className="text-blue-400">len</span>(arr) - 1<br/>
                                    &nbsp;&nbsp;<span className="text-pink-400">while</span> left &lt;= right:<br/>
                                    &nbsp;&nbsp;&nbsp;&nbsp;mid = (left + right) // 2<br/>
                                    &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-pink-400">if</span> arr[mid] == target:<br/>
                                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-pink-400">return</span> mid<br/>
                                    &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-pink-400">elif</span> arr[mid] &lt; target:<br/>
                                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;left = mid + 1<br/>
                                    <span className="text-green-600"># TODO: Implement right side logic</span>
                                 </div>
                              </div>
                              <div className="bg-[#252526] p-3 border-t border-black flex justify-between items-center">
                                 <span className="text-xs text-slate-500">Ln 8, Col 1</span>
                                 <button className="px-4 py-1.5 bg-green-600 hover:bg-green-500 text-white rounded text-xs font-bold flex items-center gap-2">
                    ...
                    {activePortalView === 'portfolio-builder' && (
                       <div className="animate-in slide-in-from-right-4 duration-300">
                           <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-2"><UserCheck className="text-orange-400"/> Student Profile Preview</h2>
                           
                           <div className="bg-slate-800 rounded-2xl p-8 border border-white/5 mb-8 flex flex-col md:flex-row gap-8 items-center md:items-start">
                              <div className="w-32 h-32 rounded-full bg-gradient-to-br from-cyan-400 to-blue-600 p-1">
                                 <div className="w-full h-full rounded-full bg-slate-900 flex items-center justify-center overflow-hidden">
                                    <img src="https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=200&q=80" className="w-full h-full object-cover" alt="Profile"/>
                                 </div>
                              </div>
                              <div className="flex-1 text-center md:text-left">
                                 <div className="flex flex-col md:flex-row justify-between items-center mb-4">
                                    <div>
                                       ...
                                       <p className="text-cyan-400">Full Stack Developer • Cloud Enthusiast</p>
                                    </div>
                                    <button className="mt-4 md:mt-0 px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-sm font-bold text-white transition-colors">Edit Profile</button>
                                 </div>
                                 <div className="flex flex-wrap gap-2 justify-center md:justify-start mb-6">
                                    <span className="px-3 py-1 rounded-full bg-slate-900 border border-white/10 text-xs text-slate-400">San Francisco, CA</span>
                                    <span className="px-3 py-1 rounded-full bg-slate-900 border border-white/10 text-xs text-slate-400">Open to Work</span>
                                    <span className="px-3 py-1 rounded-full bg-slate-900 border border-white/10 text-xs text-slate-400">Joined Jan 2024</span>
                                 </div>
                                 
                                 <h4 className="font-bold text-slate-300 text-sm uppercase tracking-wide mb-3">Earned Badges</h4>
                                 <div className="flex gap-4 justify-center md:justify-start">
                                    {[1,2,3].map((_,i) => (
                                       <div key={i} className="w-12 h-12 bg-slate-900 rounded-lg border border-yellow-500/30 flex items-center justify-center text-yellow-500 shadow-lg shadow-yellow-500/10" title="Gold Badge">
                                          <Trophy size={20}/>
                                       </div>
                                    ))}
                                 </div>
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div className="bg-slate-800 p-6 rounded-xl border border-white/5 hover:border-cyan-500/30 transition-all">
                                 <div className="flex justify-between items-start mb-4">
                                    <h4 className="font-bold text-white">E-Commerce Microservices</h4>
                                    <ExternalLink size={16} className="text-slate-500 hover:text-white cursor-pointer"/>
                                 </div>
                                 <p className="text-sm text-slate-400 mb-4">Built a scalable backend using Node.js, Docker, and Kubernetes with MongoDB sharding.</p>
                                 <div className="flex gap-2">
                                    <span className="px-2 py-1 bg-black/30 rounded text-xs text-cyan-400">Node.js</span>
                                    <span className="px-2 py-1 bg-black/30 rounded text-xs text-cyan-400">Docker</span>
                                 </div>
                              </div>
                              <div className="bg-slate-800 p-6 rounded-xl border border-white/5 flex items-center justify-center cursor-pointer hover:bg-slate-700/50 transition-all border-dashed">
        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="flex justify-between items-center h-24">
            ...
            <div className="hidden lg:flex items-center space-x-6">
              {['Home', 'About', 'Services', 'Training', 'Corporate', 'Internships', 'Staffing', 'Reviews', 'Contact'].map((item) => (
                <button 
                  key={item}
                  ...
                  className={`text-slate-300 hover:text-white font-medium transition-colors text-sm uppercase tracking-wide hover:scale-105 transform duration-200 relative group ${
                    (item.toLowerCase() === 'home' && currentView === 'home') || (item.toLowerCase() === 'about' && currentView === 'about') 
                      ? "text-white" 
                      : ""
                  }`}
                >
                  {item === 'About' ? 'About Us' : item}
                  <span className={`absolute -bottom-1 left-0 h-0.5 bg-cyan-400 transition-all group-hover:w-full ${
                    (item.toLowerCase() === 'home' && currentView === 'home') || (item.toLowerCase() === 'about' && currentView === 'about') 
                      ? "w-full" 
                      : "w-0"
                  }`}></span>
                ...
                <div className="flex flex-col lg:flex-row gap-16">
                  <div className="lg:w-1/3">
                    ...
                    <div className="bg-gradient-to-br from-purple-900/50 to-indigo-900/50 backdrop-blur-md p-6 rounded-2xl border border-white/10 mb-8 hover:bg-white/5 transition-colors shadow-lg">
                      ...
                      <button onClick={() => handleNavigation('contact')} className="text-cyan-400 text-sm font-bold hover:text-cyan-300 flex items-center gap-1">
                    ...
                    <div className="bg-gradient-to-r from-slate-900/90 to-indigo-900/90 backdrop-blur-xl p-8 md:p-10 rounded-3xl border border-white/10 shadow-2xl overflow-hidden relative mb-10">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 relative z-10">
                        {INTERNSHIP_PROGRAM.studentResources?.map((res, idx) => (
                          <div key={idx} onClick={() => handlePortalClick(res.title)} className="bg-white/5 backdrop-blur-md border border-white/10 p-5 rounded-2xl hover:bg-white/10 hover:border-cyan-400/30 transition-all group cursor-pointer h-full hover:-translate-y-1 shadow-lg">
